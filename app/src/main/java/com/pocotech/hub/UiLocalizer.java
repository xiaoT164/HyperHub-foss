...
public final class UiLocalizer {
    ...
    static {
        ...
        put("Пасхалка, предикт HyperOS 4 и бенчмарк 2.1", "Hidden mode, HyperOS 4 prediction and benchmark 2.1");
        put("История версий HyperHub", "HyperHub version history");
        put("Следить за обновлениями", "Follow updates");
        put("•  Добавлена скрытая пасхалка Hyper Egg\n•  Бенчмарк усилен: больше измерительных окон, медиана, контроль разброса и оценка доверия\n•  Список HyperOS 4 переработан: поддерживаемые, исключённые и спорные модели разделены понятнее", "•  Added a hidden Hyper Egg easter egg\n•  Benchmark upgraded: more sampling windows, median scoring, variance control and confidence estimate\n•  Reworked the HyperOS 4 list: supported, excluded and disputed devices are separated more clearly");
        put("• Убраны противоречия в списках HyperOS 4 и обновлены версии приложения/карточек интерфейса", "• Removed contradictions in HyperOS 4 device lists and refreshed app/interface card versions");
        ...
        put("v1.3.3.1 · Утилиты HyperOS", "v1.3.3.1 · HyperOS toolkit");
        ...
        put("Версия 1.3.3.1", "Version 1.3.3.1");
